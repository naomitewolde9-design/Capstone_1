...
/* We have to pull the Store_Sales (alias being ss) table, which contains all 
the information about individual sales transactions, and use JOIN, which connects 
each sale to location(state), to add Store_Locations(alias sl), which contains 
information about state, to combined the matching column "StoreID". The WHERE filters
(sl.state) 'Maryland' to exclude all non-Maryland sales. I have to use the subquery 
alias (all_sales). This helps make sure filtering happens before aggregation
Then using the GROUP BY function so that I can set the columns names as one row 
being Year and one Row being month and ORDER BY function to make the chart aligned 
and clear with support from the GROUP BY function. */

/*=======================================================================*/ 

SELECT 
	YEAR(Sale_Date) AS year, -- creates year column from the sale_date (sd alias for subquery)
	MONTH(Sale_Date) AS month, -- creates month column from the sale_date (sd alias for subquery)
...
WHERE sl.State = 'Maryland') -- filtering to find Maryland in store_loc

all_sales -- subquery alias temporary name
GROUP BY Year, Month -- combining year and month 
ORDER BY Year, Month; -- creating the column order 
...
WITH md_region AS ( -- Temporary query name 
    SELECT Region
    FROM management
...
JOIN store_locations sl
	ON ss.Store_ID = sl.StoreId) -- Select statement to match store and location from StoreID 

SELECT                        
    Location,
    SUM(Revenue) AS Total_Revenue -- connecting location to look for the sum AS total revenue column name 

FROM (SELECT -- finding Maryland 
'Maryland' AS 
...
WITH store_totals AS ( -- Temporary name 
    SELECT
        sl.StoreId,
        m.Region,
    ...
    WHERE m.Region = ( -- filtering m.Region shows the management region
...
SELECT
    StoreId,
    Region,
    Store_Revenue, -- the column order 
    RANK() OVER ( -- ranking store by revenue total
        ORDER BY Store_Revenue DESC -- most to least 
    ) AS Store_Rank -- alias 
